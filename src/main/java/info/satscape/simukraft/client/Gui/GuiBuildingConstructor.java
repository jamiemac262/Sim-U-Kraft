...
public class GuiBuildingConstructor extends GuiScreen
{
        ...
        try
        {
            ...
            else if (currentPage == 8)
            {
                ...
                while (it.hasNext())
                {
                    ...
                    if (is.getItem() != Item.getItemById(0))
                    {
                        if (y + 20 > (height - 20))
                        {
    ...
    private void showPage()
    {
        ...
        else if (currentPage == 2 || currentPage == 5 || currentPage == 6 || currentPage == 7) //what r/c/i/o building
        {
                ...
                for (int b = 0; b <= houses.size(); b++)
                {
                    ...
                    if ((boff) < houses.size())
                    {
                        if (houses.get(boff) != null)
                        {
                            ...
                            if (theWorkers.size() > 1)
                            {
                                realCost = " (" + ModSimukraft.displayMoney((float)(building.blocksInBuilding * 0.02f) * theWorkers.size()) + ")";
                            }
    ...
    long fuckingBodge = 0; // I don't know why I need to do this to stop it from fucking up.

    @Override
    public void actionPerformed(GuiButton guibutton)
    {
        if (System.currentTimeMillis() - fuckingBodge < 100)
        {
            return;
        }
        ...
        else if (currentPage == 2 || currentPage == 5 || currentPage == 6 || currentPage == 7) //chosen building to build
        {
        ...
        else if (currentPage == 8)   // building requirements (what blocks are need)
        {
            if (guibutton.displayString.contentEquals("Build it!"))
            {
                ...
                {
                    ModSimukraft.theBuildings.remove(selectedBuilding);
                }

                selectedBuilding.conBoxLocation = this.constructorLoc.clone();
                ModSimukraft.theBuildings.add(selectedBuilding);

                 Building.saveAllBuildings(); // <-server only
                selectedBuilding.saveThisBuilding();
                
                //ModSimukraft.network.sendToServer(new Message("loadbuilding"));
               // ModSimukraft.network.sendToServer(new Message("GuiBuildingCon"));

                for (int i = 0; i < theWorkers.size(); i++)
                {
                    FolkData theWorker = theWorkers.get(i);
                    theWorker.theBuilding = selectedBuilding;
                    System.out.println(theWorker.theBuilding);
                    //if (i==0) { theWorker.theBuilding.loadStructure(); }
