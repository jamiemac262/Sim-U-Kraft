...
public class Building implements Serializable {
	...
	/**
	 * get an ArrayList<V3> of all special blocks in this building that have the
	 * specified meta
	 */
	...
	private void loadStructure() {
		try {
			...
			this.ltrCount = di[0];// left to right count
			this.ftbCount = di[1];// front to back count
				...
				for (int ftb = 0; ftb < ftbCount; ftb++) {
					for (int ltr = 0; ltr < ltrCount; ltr++) {
						try {
							...
							{
								// block
								structure[acount] = Block
										.getIdFromBlock(ModSimukraft.controlBox)
										+ ":0";
							} else if (ch.contentEquals("*")) {
								// sim-u-lightbox
								structure[acount] = Block
										.getIdFromBlock(ModSimukraft.lightBox)
										+ ":0";
							} else if ((int) cha >= 48 && (int) cha <= 57) { // special
																				// block
																				// (air,
																				// but
																				// special!)
								...
								String[] sbid = structure[acount].split(":");
			...
			ModSimukraft.log
					.warning("Building loadStructure() " + e.toString());
		}
	}

	private void addToRequirements(int blockId) {
		int val = 0;
		ItemStack theBlock = new ItemStack(Item.getItemById(blockId), 1, 0); // id,
																				// size,
																				// meta
		...
		if (ModSimukraft.gameMode == GameMode.NORMAL) {
			...
			if (name.contains("planks")
					|| name.contentEquals("cobblestone")
					|| name.contentEquals("glass")
					|| name.contains("wool")
					...
					|| (name.contains("wood") && !name.contains("slab")
							&& !name.contains("door")
							&& !name.contains("stairs") && !name
								.contains("grass"))) {
	...
	private static void copyArrayList(ArrayList<Building> from,
			ArrayList<Building> to) {
	...
	public static ArrayList<Building> getBuildingBlueprints(String theType,
			String searchWords) {
			...
			if (!searchWords.contentEquals("")) {
				...
				if (searchWords.startsWith("w:")) {
					...
					for (int i = retBuildings.size() - 1; i >= 0; i--) {
						...
						if (!build.displayName.toLowerCase().contains(
	...
	public static Building getBuilding(V3 primaryXYZ) {
		Building b = null;
		System.out.println("Buildings: " + ModSimukraft.theBuildings.size());
	...
	/**
	 * overload will return an ArrayList<Building> of all buildings in world
	 * that it finds
	 */
	public static ArrayList<Building> getBuildingBySearch(String searchWord,
			boolean findAll) {
	...
	public void saveThisBuilding() {
		...
		if (this.primaryXYZ != null) {
			...
			ModSimukraft.saveSK2(ModSimukraft.getSavesDataFolder()
					+ "Buildings" + File.separator + xyz + ".sk2", strings);
	...
	public static void saveAllBuildings() {
		...
		Side side = cpw.mods.fml.common.FMLCommonHandler.instance()
				.getEffectiveSide();
		if (side == Side.SERVER) {
			ArrayList<String> strings = new ArrayList<String>();
			...
			for (int b = 0; b < ModSimukraft.theBuildings.size(); b++) {
				...
				if (building != null && building.primaryXYZ != null) {
					...
					World buildingWorld = MinecraftServer.getServer()
							.worldServerForDimension(
									building.primaryXYZ.theDimension);
					id = buildingWorld.getBlock(pxyz.x.intValue(),
							pxyz.y.intValue(), pxyz.z.intValue());
					String xyz = "b"
							+ building.primaryXYZ.toString().replaceAll(",",
									"_");
					if (id == Block.getBlockById(ModSimukraft.controlBlockId)
							|| id == Block
									.getBlockById(ModSimukraft.constructorBlockId)) {
						strings.add("displayname|" + building.displayName);
						strings.add("type|" + building.type);
						strings.add("primaryxyz|"
								+ building.primaryXYZ.toString());
						if (building.livingXYZ == null) {
							strings.add("livingxyz|null");
						} else {
							strings.add("livingxyz|"
									+ building.livingXYZ.toString());
						}
						strings.add("buildingcomplete|"
						...
						if (building.lumbermillMarker == null) {
							...
							strings.add("lmarker|"
									+ building.lumbermillMarker.toString());
						}
						strings.add("blocksinbuilding|"
						...
						} // Concurrent exception, not a proper fix!
						...
						ModSimukraft.saveSK2(ModSimukraft.getSavesDataFolder()
								+ "Buildings" + File.separator + xyz + ".sk2",
								strings);

					} else { // no control block, building destroyed in world
						File f = new File(ModSimukraft.getSavesDataFolder()
								+ "Buildings" + File.separator + xyz + ".sk2");
						if (f.exists()) {
							f.delete();
						}
			...
			ModSimukraft.log.info("Building.saveAllBuildings "
					+ ModSimukraft.theBuildings.size() + " buildings");
			...
			for (int b = 0; b < ModSimukraft.theBuildings.size(); b++) {
				...
				if (building != null) {
					if (building.primaryXYZ != null) {
						...
						// primaryxyz will be a constructor block until the
						// control
						// block has been placed
						if (id == Block
								.getBlockById(ModSimukraft.controlBlockId)
								|| id == Block
										.getBlockById(ModSimukraft.constructorBlockId)) {
							ModSimukraft.proxy.saveObject(
									ModSimukraft.getSavesDataFolder()
											+ "Buildings" + File.separator
											+ xyz + ".suk", building);
						}
					}

					// ModSimukraft.log("Saved building at "+
					// building.primaryXYZ
	...
	public static void loadAllBuildings() {
		...
		File buildingsFolder = new File(ModSimukraft.getSavesDataFolder()
				+ "Buildings" + File.separator);
		...
		if (useNewFormat) {
			System.out.println("USING NEW FORMAT==========");
			ModSimukraft.theBuildings.clear();

			for (File f : buildingsFolder.listFiles()) {
				if (f.getName().endsWith(".sk2")) {
					ArrayList<String> strings = ModSimukraft.loadSK2(f
					...
					for (String line : strings) {
						if (line.contains("|")) {
							...
							if (name.contentEquals("displayname")) {
								...
								build.buildingComplete = Boolean
										.parseBoolean(value);
								...
								build.blocksInBuilding = Integer
										.parseInt(value);
								...
								if (value.contains("B") && value.contains(",")) {
									...
									for (String block : blocks) {
										if (block.contains(",")) {
											build.blockLocations.add(new V3(
													block));
										}
									}
								}
					...
					build.loadStructure(); // fills in several more fields about
											// this building
			...
			System.out.println("USING old FORMAT==========");
			Minecraft mc = Minecraft.getMinecraft();
			//ModSimukraft.theBuildings.clear(); --- this breaks buildings 0.o

			for (File f : buildingsFolder.listFiles()) {
				if (f.getName().endsWith(".suk")) {
					Building building = (Building) ModSimukraft.proxy
							.loadObject(f.getAbsoluteFile().toString());
					...
					if (building != null) {
						...
						if (id == Block
							...
							f.delete();
							ModSimukraft.log
									.info("Building: Deleted building as id="
											+ id + " or dupe");
						}
					}
				}
	...
	/*
	 * loads in all buildings files into an ArrayList for quicker access (slowly
	 * in a thread)
	 */
	public static void initialiseAllBuildings() {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				ModSimukraft.log
						.info("Building: Thread Done Initialising all buildings from disk");
			}
		});
		t.start();
	}
	 ...
	 */
	public static Building getBuildingForFolk(String partialFilename,
			String type) {
		File f = new File(ModSimukraft.getSimukraftFolder() + "/buildings/"
				+ type + "/" + partialFilename);
		if (f.exists()) {
	...
	private static void initBuildingsOfType(String type) {
		System.out.println("initBuildingsofType");
		File f = new File(ModSimukraft.getSimukraftFolder() + "/buildings/"
				+ type);
		...
		for (int i = 0; i < f.list().length; i++) {
			...
			if (type.contentEquals("residential")) {
				...
				System.out.println("Res building added");
