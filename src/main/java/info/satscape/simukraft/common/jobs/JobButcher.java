...
public class JobButcher extends Job implements Serializable
{
    ...
    private void stageCollectingMeat()
    {
        ...
        {
            inventoriesTransferToFolk(theFolk.inventory, chestsAtFarm, new ItemStack(Items.chicken, 640),-1);
            inventoriesTransferToFolk(theFolk.inventory, chestsAtFarm, new ItemStack(Items.porkchop, 640),-1);
            inventoriesTransferToFolk(theFolk.inventory, chestsAtFarm, new ItemStack(Items.beef, 640),-1);
            step = 3;
        }
        else if (step == 3)
        {
    ...
    private void stageSellingMeat()
    {
        ...
        if (step == 1)
        {
            ...
            if (pay > 0)
            {
                ...
                ModSimukraft.sendChat(theFolk.name
                                      ...
                                      + ModSimukraft.displayMoney(pay) + " Sim-u-credits.");
                mc.theWorld.playSound(mc.thePlayer.posX,
                                      mc.thePlayer.posY, mc.thePlayer.posZ, "satscapesimukraft:cash", 1f, 1f, false);
            }
        ...
        else if (step == 3)
        {
            ...
            {
                piece = inventoriesGet(chestsAtShop, new ItemStack(Items.porkchop, 1), false,false,-1);

                if (piece == null)
                {
                    piece = inventoriesGet(chestsAtShop, new ItemStack(Items.chicken, 1), false,false,-1);
                }

                if (piece == null)
                {
                    piece = inventoriesGet(chestsAtShop, new ItemStack(Items.beef, 1), false,false,-1);
                }

                if (piece != null)
                {
                    folk = (FolkData) ModSimukraft.theFolks.get(f);
                    folk.levelFood = 10;
                    sell++;
                }
            }
            ...
            if (sell > 0)
            {
                ModSimukraft.sendChat(theFolk.name + " has sold " + sell
    ...
    *//** returns the next farm to visit or NULL if we've done all the farms *//*
    private Building getCurrentFarm()
    {
        ...
        while (!found)
        {
            try
            {
                Building farm = (Building) ModSimukraft.theBuildings.get(currentFarmNum);
                ...
                if (currentFarmNum > ModSimukraft.theBuildings.size() - 1)
                {
                    return null;
                }
            }
