...
public class JobCourier extends Job implements Serializable
{
    ...
    private void stageAtDepot()
    {
        ...
        {
            CourierTask task = ModSimukraft.theCourierTasks.get(t);
    ...
    private void stagePickingUp()
    {
        ...
        if (chests.size() == 0)
        {
            ...
            ModSimukraft.log.warning("JobCourier: StagePickingup() No chest at pickup:" + pickup.name + ", removing task.");
            currentTask++;
        ...
        else       ///chest(s) found at pickup point
        {
            ...
            ModSimukraft.log.info("JobCourier: pickupStage() "+theFolk.name + "(courier) found " + chests.size() + " chests at " + pickup.name);
            inventoriesTransferToFolk(theFolk.inventory, chests, null,ModSimukraft.lightboxId);
        }
    ...
    private void stageDroppingOff()
    {
        ...
        if (chests.size() == 0)
        {
            ModSimukraft.log.warning("JobCourierL dropoff() No chest found at dropoff");
            currentTask++;
        ...
        else      // at least one chest here
        {
            ...
            theFolk.action = FolkAction.ATWORK;
            ModSimukraft.log.info("JobCourier: "+theFolk.name + " found " + chests.size() + " chests at " + dropoff.name);

            while (theFolk.inventory.size() > 0)
            {
                int oldSize = theFolk.inventory.size();
                ModSimukraft.states.credits -= 0.11f;
                ...
                if (!placed)
                {
                    ModSimukraft.sendChat(theFolk.name + " (Courier) can't place items into chest at "
