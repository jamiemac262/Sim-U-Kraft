...
public class JobLivestockFarmer extends Job implements Serializable
{
    ...
    private void stageSlaughtering()
    {
        ...
        if (vocation == Vocation.CATTLEFARMER)
        {
            ...
            ok=inventoriesPut(farmChests, new ItemStack(Item.beefRaw, quant), true);
            inventoriesPut(farmChests, new ItemStack(Item.leather, 1), false);
        }
        else if (vocation == Vocation.PIGFARMER)
        {
            quant = rand.nextInt(2) + 1;
            ok=inventoriesPut(farmChests, new ItemStack(Item.porkRaw, quant), true);
        }
        else if (vocation == Vocation.CHICKENFARMER)
        {
            quant = rand.nextInt(2) + 1;
            ok=inventoriesPut(farmChests, new ItemStack(Item.chickenRaw, quant), true);
            inventoriesPut(farmChests, new ItemStack(Item.feather, 1), false);
        }

        if (!ok) {
        	theStage=Stage.CANTWORK;
        	ModSimukraft.sendChat(theFolk.name+"'s livestock farm chests are full!");
        	return;
        }
        
        ModSimukraft.states.credits -= (0.02f * quant);
        theStage = Stage.WAITINGFORMATUREANIMAL;
    }
