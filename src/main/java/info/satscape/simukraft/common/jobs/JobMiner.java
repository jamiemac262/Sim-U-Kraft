...
public class JobMiner extends Job implements Serializable
{
    ...
    *//**
    ...
    public JobMiner(FolkData folk)
    {
        ...
        {
            ModSimukraft.sendChat("There's a problem with the mining box that "
    ...
    private void stageWaitingForChest()
    {
        ...
        if (isChestsFull)
        {
        	if (System.currentTimeMillis() - timeSinceLastChestFullMessage > 120000)  {
        		ModSimukraft.sendChat(theFolk.name+"'s chest is full, they've stopped mining.");
            ...
            ItemStack is = new ItemStack(Block.dirt, 1);
            Boolean placedOk = inventoriesPut(miningChests, is, true);

            if (placedOk)
            {
                inventoriesGet(miningChests, is, false,false,1);
                    ...
                    try {
	                	if (theFolk.gender == 0)
	                    {
	                        mc.theWorld.playSound(theFolk.location.x,
	                                              ...
	                                              "satscapesimukraft:readym", 1f, 1f, false);
	                    }
	                    else
	                    {
	                        mc.theWorld.playSound(theFolk.location.x,
	                                              theFolk.location.y, theFolk.location.z,
	                                              "satscapesimukraft:readyf", 1f, 1f, false);
	                    }
            ...
            if (theFolk.beamingTo == null)
            {
                ...
                if (theFolk.theEntity != null)
                {
                    if (theFolk.gender == 0)
                    {
                        mc.theWorld.playSound(theFolk.location.x,
                                              theFolk.location.y, theFolk.location.z,
                                              "satscapesimukraft:beamm", 1f, 1f, false);
                    }
                    else
                    {
                        mc.theWorld.playSound(theFolk.location.x,
                                              theFolk.location.y, theFolk.location.z,
                                              "satscapesimukraft:beamf", 1f, 1f, false);
                    }
                }
                ...
                for (int ftb = 0; ftb <= ftbCount; ftb++)
                {
                    for (int ltr = 1; ltr <= ltrCount; ltr++)
                    {
                        ...
                        try {
	                        ...
	                        if (id != 0 && id != Block.waterMoving.blockID
                ...
                for (int btt = 0; btt < theMiningBox.size; btt++)
                {
                    for (int ltr = 0; ltr < theMiningBox.size; ltr++)
                    {
                        ...
                        try
                        {
                            ...
                            id = jobWorld.getBlockId(vMineable.x.intValue(),
                            ...
                            if (ftb % 10==0 && btt==Math.floor(theMiningBox.size/2) && ltr==0) {
                            	...
                            	int lbid = jobWorld.getBlockId(lightbox.x.intValue(),
                            			lightbox.y.intValue(),
                            			lightbox.z.intValue());
                            	
                            	if (miningChests.size() > 0 && lbid !=ModSimukraft.lightboxId)
                                {
                                   ItemStack light=null;
                                   int lightmeta=0;
                                   while (light==null && lightmeta <8) {
                                   	light= inventoriesGet(miningChests, new ItemStack(ModSimukraft.lightboxId,1,lightmeta), false,true,1);
                                   	lightmeta++;
                                   }
                                    if (light != null)
                                    {
                            			...
                            			,light.itemID,light.getItemDamage(),0x03);
                                    }
                                }
                            }
    ...
    *//** the actual mining stage, remove blocks, put them into the chest *//*
    private void stageMining()
    {
        ...
        jobWorld.playAuxSFX(2001, vNextMineableBlock.x.intValue(),
                            vNextMineableBlock.y.intValue(), vNextMineableBlock.z.intValue(), id + (idmeta << 12));
        ...
        jobWorld.setBlock(
            vNextMineableBlock.x.intValue(),
            vNextMineableBlock.y.intValue(),
            vNextMineableBlock.z.intValue(), 0, 0, 0x03);
        ...
        if (ModSimukraft.gameMode != GameMode.CREATIVE)
        {
            ModSimukraft.states.credits -= (0.012f);
            ...
            if (b4 != aft)
            {
                ModSimukraft.sendChat(theFolk.name + " has just levelled up to Miner Level " + aft);
            }
        }
        else
        {
        ...
        if (theFolk.employedAt.y - vNextMineableBlock.y > 3)
        {
                    ...
                    && mineHorizontalDir.contentEquals(""))
            {
                ...
                if (gid == 0)
                {
                    if (miningChests.size() > 0)
                    {
                        ItemStack glass = inventoriesGet(miningChests, new ItemStack(Block.glass, 1), false,false,1);
        ...
        if (!placedOk)
        {
            ...
            theStage = Stage.WAITINGFORCHEST;
            theFolk.inventory.clear();
            theFolk.inventory.add(new ItemStack(id, idmeta, 1));
        }
