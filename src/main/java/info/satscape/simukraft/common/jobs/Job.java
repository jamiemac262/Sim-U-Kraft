...
/*TODO: reimplement imported class then uncoment this*/ //import info.satscape.simukraft.common.TileEntityWindmill;
...
public abstract class Job {
	...
	/**
	 * overridden by subclass, fires when they were on their way to work, but
	...
	/**
	 * called repeatedly throughout working period, overridden in sub-classes of
	 * jobs
	 */
	...
	/**
	 * logic called by the subclass to get the folk to work, deals with walking
	 * to work properly
	 */
	public void onUpdateGoingToWork(FolkData theFolk) {
		if (jobWorld == null) {
			try {
				jobWorld = FMLCommonHandler
						.instance()
						.getMinecraftServerInstance()
						.worldServerForDimension(
								theFolk.employedAt.theDimension);
			} catch (Exception e) {
				e.printStackTrace();
				return;
			}
		...
		if (theFolk.pregnancyStage > 0.0f) // maternity leave for pregnant
											// ladies
		{
			return;
		}
	...
	/**
	 * return a count of how much of a particular item/block is in the folks
	 * inventory
	 */
	public int getInventoryCount(FolkData theFolk, Item wheat) {
		int ret = 0;

		for (int i = 0; i < theFolk.inventory.size(); i++) {
			ItemStack is = theFolk.inventory.get(i);

			if (is.getItem() == Items.wheat) {
				ret += is.stackSize;
			}
		}
	...
	/**
	 * find the nearest furnace to a location and return it or NULL if no
	 * furnace
	 */
	public TileEntityFurnace findFurnace(V3 v) {
		...
		if (vRet != null) {
			World theWorld = MinecraftServer.getServer()
					.worldServerForDimension(vRet.theDimension);
			ret = (TileEntityFurnace) theWorld.getTileEntity(vRet.x.intValue(),
					vRet.y.intValue(), vRet.z.intValue());
		}
	...
	private static boolean inventoryPut(IInventory chest, ItemStack inStack) {
		...
		if (inStack == null || inStack.getItem() == Item.getItemById(0)) {
			return true;
		}
			...
			for (int g = 0; g < chest.getSizeInventory(); g++) {
				...
				if (is == null) {
					...
					if (isTest != null) {
						...
						ModSimukraft.log
								.warning("Job: placeIntoInventory() could not place "
										+ is.getDisplayName()
										+ " in null slot " + g);
						placedOK = false;
					}
				} else if (is.getItem() == inStack.getItem()
						&& is.getItemDamage() == inStack.getItemDamage()
					...
					if (isAfter > isBefore) {
						...
						ModSimukraft.log
								.warning("Job: placeIntoInventory() could not inc Stacksize for "
										+ is.getDisplayName() + " in slot " + g);
						placedOK = false;
					}
				}
			}
	 ...
	 * 
	 * @param chest
	 *            - the chest or inventory
	 * @param whatItem
	 *            - the Item type/quantity required etc or NULL to get ANY item
	 * @param getRandomItem
	 *            - used if whatItem is NULL - if false, get the first available
	 *            item, if true, gets a random item
	 * @return the Itemstack of what was pulled out, or NULL if it was unable to
	 *         get the item
	 */
	public static ItemStack inventoriesGet(ArrayList<IInventory> chests,
			ItemStack whatItem, boolean getRandomItem, boolean compareMeta,
			int ignoreId) {
		ItemStack retStack = null;

		for (int c = 0; c < chests.size(); c++) {
			IInventory chest = chests.get(c);
			retStack = inventoryGet(chest, whatItem, getRandomItem,
					compareMeta, ignoreId);
	...
	// called by public function
	private static ItemStack inventoryGet(IInventory chest, ItemStack whatItem,
			boolean getRandomItem, boolean compareMeta, int ignoreId) {
		if (whatItem != null) {
			...
			for (int g = 0; g < chest.getSizeInventory(); g++) {
				...
				if (ignoreId > -1) {
					if (chestStack.getItem() == Item.getItemById(ignoreId)) {
						ignore = true;
					}
				}
			...
			if (getRandomItem) {
				...
				returnStack = chest.getStackInSlot(new Random().nextInt(slots
						.size()));
				return returnStack;
			} else {
				ItemStack returnStack = null;
	...
	/**
	 * used to place a itemStack into a set of chests or other inventories, will
	 * use all if needed, return false if it was unable to place into any of the
	 * chests specified due to being full
	 */
	public boolean inventoriesPut(ArrayList<IInventory> chests,
			ItemStack inStack, boolean doOpenClose) {
		boolean placedOK = false;
	...
	public static boolean inventoriesPut2(ArrayList<IInventory> chests,
			ItemStack inStack) {
		boolean placedOK = false;
	 ...
	 * 
	 * @param folkInventory
	 *            the folks inventory to transfer
	 * @param toChests
	 *            the set of chests to transfer into
	 * @param specifcItems
	 *            NULL if any/all items or specify an itemStack if they should
	 *            only place certain things
	 * @return true if successful, false if chests are all full
	 */
	public boolean inventoriesTransferFromFolk(
			ArrayList<ItemStack> folkInventory, ArrayList<IInventory> toChests,
		...
		for (int i = 0; i < folkInventory.size(); i++) {
			try {
				...
				if (specificItems != null
						&& specificItems.getItem() == folkStack.getItem()) {
					okToPlace = true;
				...
				if (okToPlace) {
					...
					if (!placed) {
						ModSimukraft.log
								.warning("Job: Could not place stack of "
										+ folkStack.getDisplayName()
										+ " in chest");
						return false;
					}
				}
	 ...
	 * 
	 * @param folkInventory
	 *            The folks inventory to transfer into
	 * @param fromChests
	 *            the chests where to get them
	 * @param whatItems
	 *            NULL for any item or an itemstack (inc. quantity) of what they
	 *            do want
	 * @param ignoreId
	 *            pass in -1 to not ignore any blocks or the blockID of stuff to
	 *            leave in chest
	 * @return true for successfully got at least one stack, false if nothing
	 *         got
	 */
	public boolean inventoriesTransferToFolk(
			ArrayList<ItemStack> folkInventory,
			ArrayList<IInventory> fromChests, ItemStack whatItems, int ignoreId) {
		boolean ret = false;
		int limit = 0;
		ItemStack got = null;
	...
	/**
	 * transfer a limited amount of a particular item from a set of chests
	 * (IInventory) into the folk's inventory
	 */
	public int inventoriesTransferLimitedToFolk(
			ArrayList<ItemStack> folkInventory,
			ArrayList<IInventory> fromChests, ItemStack whatItems, int getQty,
			boolean doCompareMeta) {
		int gotSoFar = 0;
		for (IInventory chest : fromChests) {
			for (int g = 0; g < chest.getSizeInventory(); g++) {
				...
				if (chestStack != null && chestStack.getItem() == whatItems.getItem()) {
					if (doCompareMeta) {
						if (chestStack.getItemDamage() == whatItems
								.getItemDamage()) {
							gotMatch = true;
						}
					} else {
				...
				if (gotMatch) {
					while (gotSoFar < getQty && chestStack.stackSize > 0) {
						gotSoFar++;
						chestStack.stackSize--;
						folkInventory.add(new ItemStack(chestStack.getItem(), 1,
								chestStack.getItemDamage()));
					}
					if (chestStack.stackSize > 0) {
	...
	/**
	 * returns an int Count of how many of the passed in item are in the chests
	 * (counts only, does not take out)
	 */
	public int getItemCountInChests(ArrayList<IInventory> chests, ItemStack is,
			boolean doCompareMeta) {
		int ret = 0;
	...
	/**
	 * sets the closestBlocks arraylist with V3 locations in world where the
	 * passed in Block types are found
	 */
	public void setClosestBlocksOfType(final V3 startXYZ,
			final ArrayList<Integer> blockIDs, final int distanceLimit,
			final boolean needsToSeeSky, final boolean scanDownwards,
			final boolean oneLayerOnly) {
		Thread t = new Thread(new Runnable() {
			public void run() {
				World theWorld = MinecraftServer.getServer()
						.worldServerForDimension(startXYZ.theDimension);
						...
						for (int xo = -d; xo <= d; xo++) {
							for (int zo = -d; zo <= d; zo++) {
								...
								for (int b = 0; b < blockIDs.size(); b++) {
									...
									Block blockInWorld = theWorld.getBlock(sx,
											sy, sz);

									if (blockInWorld == Block.getBlockById(blockID)) {
										if (needsToSeeSky) {
											boolean canSeeSky;

											if (theWorld.getBlock(sx, sy + 1,
													sz) == Blocks.air) {
												canSeeSky = true;
											} else {
												canSeeSky = false;
	...
	/**
	 * find all the closest inventories/chests within the search area and
	 * returns an arraylist of chest-type things
	 */
	public static ArrayList<IInventory> inventoriesFindClosest(V3 startXYZ,
			int searchDistance) {
		...
		try {
			World theWorld = MinecraftServer.getServer()
					.worldServerForDimension(startXYZ.theDimension);
			TileEntity te = theWorld.getTileEntity(startXYZ.x.intValue(),
					startXYZ.y.intValue(), startXYZ.z.intValue());

			if (te != null) {
				if (te instanceof IInventory
						&& !(te instanceof TileEntityFurnace)
						/*TODO: re-implement*/
						/*&& !(te instanceof TileEntityWindmill)*/) {
	...
	private static boolean alreadyGotChest(ArrayList<IInventory> chests,
			IInventory chest) {
		boolean ret = false;
	...
	public static V3 findAdjacentSpace(V3 startXYZ, World world) {
		...
		if (theWorld == null) {
			theWorld = MinecraftServer.getServer().worldServerForDimension(
					startXYZ.theDimension);
		}

		V3 test = startXYZ.clone();
		test.x++;
		if (theWorld.isAirBlock(test.x.intValue(), test.y.intValue(),
				test.z.intValue())) {
			return test;
		}
		test = startXYZ.clone();
	...
	/**
	 * searches up to 10 below the location and a distance of up to 80 for a
	 * certain type of block will return null if none is found in that area
	 */
	public static V3 findClosestBlockType(V3 startXYZ, Block block,
			int searchDistance, boolean mustSeeSky) {
		World theWorld = MinecraftServer.getServer().worldServerForDimension(
				startXYZ.theDimension);
		// ModSimukraft.log("findClosestBlockType: Id="+block.blockID);

		if (theWorld.getBlock(startXYZ.x.intValue(), startXYZ.y.intValue(),
				startXYZ.z.intValue()) == block) {
			return startXYZ;
		}
				...
				for (int xo = -d; xo <= d; xo++) {
					for (int zo = -d; zo <= d; zo++) {
						...
						if (theWorld.getBlock(sx, sy, sz) == block) {
							V3 ret = new V3((double) sx, (double) sy,
									(double) sz, startXYZ.theDimension);
							return ret;
						} else {
	...
	/**
	 * searches on the same Y level only within the searchDistance for the block
	 * type and return V3 or null
	 */
	public static V3 findClosestBlockType(V3 startXYZ, Block block,
			int searchDistance) {
		World theWorld = MinecraftServer.getServer().worldServerForDimension(
		...
		for (int d = 1; d < searchDistance; d++) {
			for (int xo = -d; xo <= d; xo++) {
				for (int zo = -d; zo <= d; zo++) {
					...
					if (theWorld.getBlock(sx, sy, sz) == block) {
						V3 ret = new V3((double) sx, (double) sy, (double) sz,
								startXYZ.theDimension);
						return ret;
					}
				}
	...
	/**
	 * find an arraylist of locations where the specified block is found sorted
	 * into closest first
	 */
	public static ArrayList<V3> findClosestBlocks(V3 startXYZ, Block block,
			int distanceLimit) {
			...
			for (int xo = -distanceLimit; xo <= distanceLimit; xo++) {
				for (int zo = -distanceLimit; zo <= distanceLimit; zo++) {
					try {
						...
						if (theWorld.getBlock(sx, sy, sz) == block) {
							V3 v = new V3((double) sx, (double) sy,
									(double) sz, startXYZ.theDimension);
		...
		for (int i = 0; i < blocksFound.size(); i++) {
			...
			double distance = Math.sqrt((v.x - startXYZ.x) * (v.x - startXYZ.x)
					+ (v.z - startXYZ.z) * (v.z - startXYZ.z));
	...
	/*
	 * tries to mine a block at xyz and places mined stuff into chests, does
	 * translations (Coal ore > coal item) Returns true if something was mined
	 */
	public boolean mineBlockIntoChests(ArrayList<IInventory> chests, V3 blockXYZ) {
		boolean ret = false;
		ArrayList<ItemStack> minedStacks = translateBlockWhenMined(jobWorld,
				blockXYZ);
	...
	/**
	 * scans a 3x3 area of the pen to count how many animals are in it (pass in
	 * Class of animal)
	 */
	public int getAnimalCountInPen(V3 controlBox, Class animal) {
		List list = jobWorld.getEntitiesWithinAABB(
				animal,
				AxisAlignedBB.getBoundingBox(controlBox.x, controlBox.y,
						controlBox.z, controlBox.x + 1.0D, controlBox.y + 1.0D,
						controlBox.z + 1.0D).expand(3D, 2D, 3D));// look within
																	// the pen
	...
	public enum Vocation {
		BUILDER, LUMBERJACK, MINER, CROPFARMER, BAKER, SOLDIER, SHEPHERD, GROCER, COURIER, MERCHANT, BUTCHER, PIGFARMER, CATTLEFARMER, CHICKENFARMER, TERRAFORMER, GLASSMAKER, FISHERMAN, PATHBUILDER, DAIRYFARMER, CHEESEMAKER, BURGERSMANAGER, BURGERSFRYCOOK, BURGERSWAITER;
