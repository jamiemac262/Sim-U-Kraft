...
import info.satscape.simukraft.common.jobs.JobShepherd.Stage;
...
public class JobFisherman extends Job implements Serializable
{
    ...
    private void stageArrived()
    {
        ...
        if (water == null)
        {
            ...
            ModSimukraft.sendChat(theFolk.name + " (Fisherman) can't find any fish in the area");
        }
        else
        {
    ...
    private void stageFishing()
    {
        ...
        if (mc.getServer().worldServers[0].getWorldTime() % 24000 > 11980)
        {
    ...
    private void stageCaughtFish()
    {
        ...
        if (dockChests.size() == 0)
        {
            ...
            ModSimukraft.sendChat(theFolk.name + " (fisherman) can't find any chests at the dock!");
    ...
    {
        if (mc.getServer().worldServers[0].getWorldTime() % 24000 < 11600)
        {
            theStage = Stage.IDLE;
            return;
        }
        ...
        if (step == 1)
        {
            ...
            if (ModSimukraft.theFolks.size() > 1)
            {
                ...
                dockChests = inventoriesFindClosest(theFolk.employedAt, 4);
                fishStack = inventoriesGet(dockChests, new ItemStack(Items.fish, sell), false,false,-1);
            }
            ...
            if (fishStack == null)
            {
                //ModSimukraft.sendChat(theFolk.name +" did not have any fish to sell today, might do better tomorrow.");
                theStage = Stage.IDLE;
                return;
            }
            else
            {
                ModSimukraft.sendChat(theFolk.name + " caught " + fishCount + " fish today and has sold "
