...
public class JobBaker extends Job implements Serializable {
	...
	private void stageCollectingWheat() {
		...
		if (step == 1) {
			...
			farmChests = inventoriesFindClosest(farm.getLocation(), 5);
			inventoriesTransferToFolk(theFolk.inventory, farmChests,
					new ItemStack(Items.wheat, 640), -1);
			step = 3;
		} else if (step == 3) {
			...
			if (theFolk.theEntity != null) {
				if (theFolk.gender == 0) {
					mc.worldServers[0].playSound(theFolk.location.x,
							...
							"satscapesimukraft:bakerm", 1f, 1f, false);
				} else {
					mc.worldServers[0].playSound(theFolk.location.x,
							theFolk.location.y, theFolk.location.z,
							"satscapesimukraft:bakerf", 1f, 1f, false);
				}
			}
	...
	private void stageSellingBread() {
		if (step == 1) {
			if (pay > 0) {
				...
				mc.worldServers[0].playSound(mc.getPlayerCoordinates().posX, mc.getPlayerCoordinates().posY,
						mc.getPlayerCoordinates().posZ, "satscapesimukraft:cash", 1f, 1f,
						false);
			}
			...
			if (ModSimukraft.theFolks.size() > 1) {
				sell = ModSimukraft.theFolks.size() + 1
						+ new Random().nextInt(ModSimukraft.theFolks.size());
				bakeryChests = inventoriesFindClosest(theFolk.employedAt, 4);
				breadStack = inventoriesGet(bakeryChests, new ItemStack(
						Items.bread, sell), false, false, -1);
			}
			...
			if (breadStack == null) {
				ModSimukraft
						.sendChat(theFolk.name
								+ " did not have any bread to sell today, do you have an active wheat farm?");
			} else {
				ModSimukraft.sendChat(theFolk.name + " has sold "
						+ breadStack.stackSize
						+ " loafs of bread to folks today.");
	...
	private FarmingBox getCurrentFarm() {
		...
		while (!found) {
			try {
				farm = (FarmingBox) ModSimukraft.theFarmingBoxes
						.get(currentFarmNum);
