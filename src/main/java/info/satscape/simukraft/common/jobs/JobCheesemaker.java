...
public class JobCheesemaker extends Job {
    ...
    private void openCloseMilkShutters(boolean open) {
    	ArrayList<V3> shutters=theCheeseFactory.getSpecialBlocks(2);
    	if (shutters==null || shutters.size()<1) {
    		theFolk.selfFire();
        	ModSimukraft.sendChat("There was a problem with the Cheese factory, try re-building it");
        	return;
    	}
    	int newId=0;
    	if (open) {
    		...
    		newId=Block.getIdFromBlock(Blocks.iron_block);
    	}
    	
    	Block current=jobWorld.getBlock(shutters.get(0).x.intValue(),shutters.get(0).y.intValue(),shutters.get(0).z.intValue());
    	
    	if (current != Block.getBlockById(newId)) {
    		if (open && current==ModSimukraft.blockFluidMilk) { return; } //opening and it already has milk flowing
     		
			for(V3 v3:shutters) {
				jobWorld.setBlock(v3.x.intValue(),v3.y.intValue(),v3.z.intValue(),Block.getBlockById(newId),0,0x03);
			}
			mc.worldServers[0].playSound(theFolk.location.x,
	                theFolk.location.y, theFolk.location.z,
	                "satscapesimukraft:cheesemachine", 1f, 1f, false);
    	}
    }
	...
	private void stageArrivedAtFactory() {
		...
		inventoriesTransferToFolk(theFolk.inventory, chests, new ItemStack(Items.milk_bucket,64),-1);
		} catch(Exception e) {}
	...
	private void stageGoingToDairyFarm() {
		...
		if (!dairyFarms.isEmpty() && (dairyFarms.size()-1) <=currentFarmNum) {
			...
			if (dairyFarms.isEmpty()) {
				ModSimukraft.sendChat(theFolk.name+" has retired, as there are no dairy farms");
				theFolk.selfFire();
				return;
	...
	private void stageCollectingMilk() {
		...
		if (step==1) {
			...
			if (chestsAtDairy.isEmpty()) {
	        	ModSimukraft.sendChat(theFolk.name+": Can't find any chests at the dairy farm, I quit!");
	        	theFolk.selfFire();
	        	return;
			}
			
			//will grab it all, but in single inventory slots
			inventoriesTransferToFolk(theFolk.inventory, chestsAtDairy, new ItemStack(Items.milk_bucket,1),-1); 
			
			if (theFolk.inventory ==null || theFolk.inventory.isEmpty()) {
				ModSimukraft.sendChat(theFolk.name +" hasn't found any milk at the dairy today.");
	...
	private void stageEmptyingMilk() {
		if (step==1) {
			...
			for(V3 milkBlock:milkblocks) {
				...
				if (id== Block.getBlockById(0) || (id ==ModSimukraft.blockFluidMilk && meta ==1)) {
					jobWorld.setBlock(milkBlock.x.intValue(), milkBlock.y.intValue(), milkBlock.z.intValue()
							, ModSimukraft.blockFluidMilk, 0, 0x03);
					try {theFolk.inventory.remove(0);}catch(Exception e){}  //just in case!
					filledOk=true;
					break;
				}
			}
	...
	private void transformMilkToCheese(V3 currentStirPos) {
		...
		for(int m=milkBlocks.size()-1;m>0;m--) {
			...
			if (id==ModSimukraft.blockFluidMilk && meta==0) {
				jobWorld.setBlock(milk.x.intValue(), milk.y.intValue(), milk.z.intValue(),Blocks.air,0,0x03);
				milkGotCount++;
			...
			for(V3 cheese:cheeseBlocks) {
				...
				if (id != ModSimukraft.blockCheese && dist<5) {
					jobWorld.setBlock(cheese.x.intValue(), cheese.y.intValue(), cheese.z.intValue()
							,ModSimukraft.blockCheese,0,0x03);
					placedCheese=true;
					break;
				}
			}
	...
	private void stageHarvestCheese() {
		...
		if (step==1) {
			...
			for(V3 block:cheeseBlocks) {
				...
				if(stirPositions.get(0).getDistanceTo(block) <5 && id==ModSimukraft.blockCheese) {
					gotBlock=true;
					theFolk.inventory.add(new ItemStack(ModSimukraft.blockCheese));
					jobWorld.setBlock(block.x.intValue(),block.y.intValue(),block.z.intValue(),Blocks.air,0,0x03);
					theFolk.isWorking=true;
			...
			for(V3 block:cheeseBlocks) {
				Block id=jobWorld.getBlock(block.x.intValue(),block.y.intValue(),block.z.intValue());
				if(stirPositions.get(1).getDistanceTo(block) <5 && id==ModSimukraft.blockCheese) {
					gotBlock=true;
					theFolk.inventory.add(new ItemStack(ModSimukraft.blockCheese));
					jobWorld.setBlock(block.x.intValue(),block.y.intValue(),block.z.intValue(),Blocks.air,0,0x03);
					break;
	...
	private void stageSliceCheese() {
		...
		if (step==1) {
			...
			if (chests.isEmpty()) {
				ModSimukraft.sendChat(theFolk.name+": Someone has stolen the chest in the cheese factory, I quit!");
				theFolk.selfFire();
			}
			inventoriesTransferFromFolk(theFolk.inventory, chests, null);
			step=4;
			...
			ItemStack cheese=inventoriesGet(chests, new ItemStack(ModSimukraft.blockCheese,1), false, false,-1);
			if (cheese !=null) {
				boolean placedOK=inventoriesPut(chests, new ItemStack(ModSimukraft.itemFood, 9, 0), true); //9 x cheese slices
				if (!placedOK) {
					ModSimukraft.sendChat(theFolk.name+"'s chest at the cheese factory is full of cheese!");
					theFolk.selfFire();
				}
			} else {
				step=5;
