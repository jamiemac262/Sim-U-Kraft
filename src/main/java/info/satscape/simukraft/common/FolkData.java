...
import info.satscape.simukraft.common.jobs.JobBurgersFryCook;
import info.satscape.simukraft.common.jobs.JobBurgersManager;
import info.satscape.simukraft.common.jobs.JobBurgersWaiter;
//import info.satscape.simukraft.common.jobs.JobButcher;
import info.satscape.simukraft.common.jobs.JobCheesemaker;
/*import info.satscape.simukraft.common.jobs.JobCourier;
import info.satscape.simukraft.common.jobs.JobCropFarmer;
import info.satscape.simukraft.common.jobs.JobDairyFarmer;
...
import info.satscape.simukraft.common.jobs.JobTerraformer.TerraformerType;*/
...
public class FolkData implements Serializable
{
   ...
   // public String partneredWith = "";
    ...
    protected transient float matingStage = -1.0f; // -1 not had today  0.0 to 0.9=having     1.0=had
    private transient EntityFolk entityId;
    ...
    public void hasLoaded()
    {
        ...
        try {
        ModSimukraft.log.info("FolkData: hasLoaded() " + this.name + " (" + voc + ") at "
                         + vocat + " location= " + this.location.toString() + "  "  + ModSimukraft.theFolks.size() + " folks in total");
    ...
    public void respawnEntity(World world)
    {
        ...
        if (getDistanceToPlayer() < 50)
        {
            ...
            entityId = this.theEntity;
            ModSimukraft.log.info("FolkData:repawnEntity() " + this.name + " at " + this.location.toString() + " in dim " + this.location.theDimension + " ENTITY:" + this.theEntity);
        }
    }
    ...
    /** runs client side receiving updates where this folk actual is - NOT IMPLIMENTED? */
    ...
    public void onUpdate()
    {
        ...
        if (now - timeSinceLastMinute > 60000)
        {
            ...
            if (t<2000 && this.pregnancyStage>=1.0f) {
            	for(Building build:ModSimukraft.theBuildings) {
            ...
            if (action == FolkAction.WANDER && this.isSpawned() && this.employedAt == null && this.age >= 18 && !this.statusText.contains("baby"))
            {
                ...
                {
                    Building b = ModSimukraft.theBuildings.get(rand.nextInt(ModSimukraft.theBuildings.size()));
                    ...
                    if (b.type.contentEquals("commercial") && dist < 40)
                    {
                        ...
                        for (int f = 0; f < ModSimukraft.theFolks.size(); f++)
                        {
                            FolkData keeper = ModSimukraft.theFolks.get(f);
                        ...
                        if (hasShopKeeper)
                        {
                            ModSimukraft.log.info("FolkData:onUpdate() "+name + " is wandering to " + b.displayName + " " + dist + " blocks away.");
                ...
                if (!gotWanderPoint)
                {
                    ...
                    while (world.getBlock(wanderTo.x.intValue(), wanderTo.y.intValue(), wanderTo.z.intValue()) != Blocks.air
                            && wanderTo.y < 255)
                    {
                        wanderTo.y++;
                    }
            ...
            if (!ModSimukraft.isDayTime() && Relationship.isFolkLivingWithSomeone(this) && matingStage < 0)
            {
                tryForBaby();
            }
        ...
        if (now - timeSinceLastStatusUpdate > 1000)
        {
            ...
            if (ModSimukraft.isDayTime() && this.employedAt != null &&
            ...
            if (pregnancyStage > 0.0f && this.employedAt != null && ModSimukraft.isDayTime())
            {
                this.statusText = "On Maternity leave";
            }

            if (ModSimukraft.isDayTime() && this.employedAt != null && this.action != FolkAction.ATWORK
            ...
            if (ModSimukraft.isDayTime() && this.employedAt == null && action == FolkAction.ATHOME)
            {
                isWorking = false;
                    ...
                    && this.destination == null && this.theEntity != null)
            {
                ...
                if (talkCounter == 12)
                {
                    if (ModSimukraft.configFolkTalking)
                    {
                        ...
                        String letter = "satscapesimukraft:blarg" + Character.toString((char) ch);
                        ModSimukraft.proxy.getClientWorld().playSound(location.x, location.y, location.z, letter, 1.0f, 1.0f, false);
                        //ModSimukraft.log(this.name + " Spoke: " + letter);
                    }
    ...
    private void tryForBaby()
    {
        if (this.gender == 1 && this.pregnancyStage == 0.0f) //only need to do this for non-preg females
        {
            ...
            if (malePartner != null && this.action == FolkAction.ATHOME && malePartner.action == FolkAction.ATHOME)
            {
                ...
                //ModSimukraft.log("tryForBaby() called");
            }
        }
        else
        {
        ...
        if (this.action == FolkAction.WANDER)
        {
            ...
            for (int b = 0; b < ModSimukraft.theBuildings.size(); b++)
            {
                        ...
                        && building.type.contentEquals("residential"))
                {
                   ...
                   //ModSimukraft.states.population++;
            ...
            for (int inv = 0; inv < inventory.size(); inv++)
            {
                ...
                if (is != null)
                {
                    if (theEntity != null)
                    {
                        try
                        {
                            theEntity.dropItem(is.getItem(), is.stackSize);
                        }
                        catch (Exception e) {}
                    }
                    else
                    {
                        try
                        {
                            FolkData.getClosestPlayer(this.location).dropItem(is.getItem(), is.stackSize);
                        }
                        catch (Exception e) {}
                    }
    ...
    public void gotoXYZ(V3 whereTo, GotoMethod methodOfTravel)
    {
        ...
        try
        {
            ModSimukraft.log.info("FolkData: GOTOXYZ() for " + this.name + " to " + whereTo.toString() + " - Method:" + gotoMethod.toString()
    ...
    public void beamMeTo(V3 whereToIn)
    {
        ...
        try {
	        ...
	        {
	            ModSimukraft.proxy.getClientWorld().playSound(location.x, location.y, location.z, "satscapesimukraft:beamdown", 1.0f, 1.0f, false);
	            ModSimukraft.proxy.getClientWorld().playSound(whereTo.x, whereTo.y, whereTo.z, "satscapesimukraft:beamdown", 1f, 1f, false);
	        }
    ...
    /** called repeatadly during the beaming progress */
    private void doBeaming()
    {
        try
        {
            if (System.currentTimeMillis() - timeStartedGotoing > 4000 || beamingTo == null)
            {
                ...
                ModSimukraft.log.info("FolkData: doBeaming() complete for " + this.name + " to " + beamingTo.toString() + " (dim " + beamingTo.theDimension + ")");
	            		...
	            		if (line.contains("|")) {
	            			...
	            			try {
	        	    			if (name.contentEquals("employedat")) {
	        	    				...
	        	    				if (!value.contentEquals("null")) {
	        	    					//TODO: folkd.terraformerType=TerraformerType.valueOf(value);
	        	    				}
    ...
    public Building getHome()
    {
        for (int b = 0; b < ModSimukraft.theBuildings.size(); b++)
        {
            Building home = ModSimukraft.theBuildings.get(b);
    ...
    {
    	System.out.println("GenerateNewFolkData called!");
    ...
    public static FolkData getFolkDataByEntityId(int id)
    {
        ...
        {
            FolkData fd = ModSimukraft.theFolks.get(i);
    ...
    public void eventDied(DamageSource d)
    {
        ...
        ModSimukraft.sendChat(this.name
                              + oldJob + " has just died! " + deathBy + only);
        this.action = FolkAction.WANDER;
        int i = 0;

        //find the folk in the array
        for (int fn = 0; fn < ModSimukraft.theFolks.size(); fn++)
        {
            FolkData fo = ModSimukraft.theFolks.get(fn);
        ...
        try
        {
            ...
            for (int q = 0; q < ModSimukraft.theRelationships.size(); q++)
            {
                try
                {
                    Relationship rel = ModSimukraft.theRelationships.get(q);
                    ...
                    if (rel.folk1.name.contentEquals(this.name) || rel.folk2.name.contentEquals(this.name))
                    {
                        ...
                        File f = new File(ModSimukraft.getSavesDataFolder() + "Relationships" + File.separator
                                          + fn + ".sk2");
                        f.delete();
            ...
            if (i >= 0)
            {
                if (i < ModSimukraft.theFolks.size())
                {
        ...
        {
            ModSimukraft.log.warning("FolkData: eventDied() "+e.toString());
